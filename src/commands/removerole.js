...
module.exports = {
    data:
        new SlashCommandBuilder()
            .setName('removerole')
            .setDescription(`Removes someone a role!`)
            .addUserOption((option) =>
                option
                    .setName("user")
                    .setDescription("The User that you want to remove a role to.")
                    .setRequired(true)
            )
    ...
    run: async ({ interaction }) => {
        ...
        if (!user.roles.cache.has(role.id)) {
            return interaction.reply({ content: `The member ${user.user} doesn't have the role ${role}.`, ephemeral: true });
        }
        ...
        try {
            ...
            interaction.reply({ content: `An Error Occurred. The role that you're trying to remove might be above me in the role hierarchy.\nError:\n${err}`, ephemeral: true });
        }
