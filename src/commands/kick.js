...
module.exports = {
    ...
    run: ({ interaction, client }) => {
        ...
        if (user.roles.highest.position >= interaction.member.roles.highest.position) {
            return interaction.reply({ content: `You can't kick someone with higher or same position as you.`, ephemeral: true });
        }
        ...
        if (!user.kickable) {
            return interaction.reply({ content: `I can't kick this user. He might be above me in the role hierarchy.`, ephemeral: true });
        }
