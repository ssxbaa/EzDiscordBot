...
module.exports = {
    data:
        new SlashCommandBuilder()
            .setName('mute')
            .setDescription('Mutes a guild in the User.')
            ...
            .addStringOption((option) =>
                option
                    .setName("time")
                    .setDescription("The time that you want to mute the User for.")
                    .setRequired(true)
            )
            .addStringOption((option) =>
                option
    ...
    run: async ({ interaction, guild, client }) => {
        ...
        if (user.roles.highest.position >= interaction.member.roles.highest.position) {
            return interaction.reply({ content: `You can't mute someone with higher or same position as you.`, ephemeral: true });
        }
        ...
        if (!user.moderatable) {
            return interaction.reply({ content: `I can't mute this user. He might be above me in the role hierarchy.`, ephemeral: true });
        }
